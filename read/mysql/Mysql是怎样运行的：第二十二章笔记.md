...
还记得 undo_demo 表的主键 id 列是什么类型吗？是的，INT类型，而 INT 类型占用的存储空间长度就是 4 个字节，这就是 4 的由来。
...
我们的回滚操作是针对删除操作的阶段一（delete mark 阶段）进行的，因为阶段二意味着事务的提交，事务提交后我们就不用回滚了。

InnoDB 设计了一种称为 TRX_UNDO_DEL_MARK_REC 类型的 undo 日志，他的结构如下（各个部分按序号顺序，从上到下排列）：

1. end of record：本条 undo 日志结束，下一条开始时在页面中的地址。
2. undo type：本条 undo 日志的类型，也就是 TRX_UNDO_DEL_MARK_REC。
3. undo no：本条 undo 日志对应的编号。
4. table id：本条 undo 日志对应记录所在表的 table id。
5. info bits：记录头信息前 4 个比特位的值以及 record_type 的值。
6. old trx_id：记录旧的 trx_id 值。
7. old roll_pointer：记录旧的 roll_pointer 值。
8. 主键各列信息（结构为 <len, value>）：主键的**每个列**占用的存储空间大小（即 len）和真实值（即 value）。
9. index_col_info len：**索引列各列信息**部分和 **index_col_info len** 部分占用的存储空间大小。
10. 索引列各列信息（结构为 < pos, len, value>）：索引列的**每个列（凡是被索引的列）**在记录中的位置（即 pos）、该列占用的存储空间大小（即 len）和该列的实际值（即 value）
11. start of record：上一条 undo 日志结束，本条开始时在页面中的地址。

这里我们需要强调几点：

* 在对一条记录进行 delete mark 操作**之前**，需要把该记录的旧的 trx_id 和 roll_pointer 隐藏列的值，都记录到对应 undo 日志的 old trx_id 和 old roll_pointer 属性中，这样做可以让我们通过 undo 日志的 old roll_pointer 找到记录在修改前对应的 undo 日志。例如，我们在一个事务中，先插入一条记录再删除该记录，过程如下：

  1. 先插入一条记录，页面中便存在了一条正常记录如下：

     ```mysql
     -- 新插入的记录的格式（简化版），记为记录 A
     +-----------+-----------+-----------------------------------+
     |delete_mask|   type    |      roll_pointer                 |
     +-----------+-----------+-----------------------------------+
     |     0     |  正常记录  | 指向 insert undo 日志（记为日志 A） |
     +-----------+-----------+-----------------------------------+
     ```

  1. 执行删除操作，删除记录 A，记录 A 的状态发生变更
  
     ```mysql
     -- 执行删除操作，记录 A 的状态发生变更
     -- 此时记录 A 的 roll_pointer 指向日志 B，而日志 B 的 old roll_pointer 属性指向日志 A
     +-----------+----------+----------------------------------+
     |delete_mask|   type   |          roll_pointer            |
     +-----------+----------+----------------------------------+
     |     1     |中间状态记录| 指向 delete undo 日志（记为日志 B）|
     +-----------+----------+----------------------------------+
     ```
  
     可以看到，在执行完 delete mark 操作后，日志 A 和日志 B 串成了一个链表，这个链表被称为**版本链**。
