...
为了保证事务的持久性，**用户线程**在事务提交时需要将该事务执行过程中产生的所有 redo 日志都刷新到磁盘上，但这严重影响数据库性能。

为了解决上述问题，存在一个**系统变量 innodb_flush_log_at_trx_commit**，它有三个值，分别如下：

* 当系统变量 innodb_flush_log_at_trx_commit 的值为 0 时，表示在事务提交时不立即向磁盘中同步 redo 日志，这个任务是交给**后台线程**去做的。

  明显的，这样子做会加快请求的处理速度。但是后台线程若没有及时的将事务提交产生的 redo 日志刷新到磁盘，那么事务对页面的修改就会因为服务器的崩溃而丢失。

* 当系统变量 innodb_flush_log_at_trx_commit 的值为 1 时（该值是系统变量 innodb_flush_log_at_trx_commit 的默认值），表示在事务提交时需要将 redo 日志同步到磁盘，这样做保证了事务的持久性。

* 当系统变量 innodb_flush_log_at_trx_commit 的值为 2 时，表示在事务提交时需要将 redo 日志**写到操作系统的缓冲区中，但不需要保证将 redo 日志真正的刷新到磁盘**。

  这样就算数据库崩溃了，只要系统没崩溃，就可以保证事务的持久性。
...
我们接下来看一下服务器崩溃重启时，redo 日志是如何帮助我们恢复状态的。
...
对于 checkpoint_lsn 之前的 redo 日志，我们不必关心，因为这些日志对应的脏页都已经被刷新到了磁盘。而对于 checkpoint_lsn 之后的 redo 日志，这些日志对应的脏页可能被刷盘了也可能没有，无法确定。因此，我们需要从 checkpoint_lsn 开始读取 redo 日志来恢复页面。

我们知道，redo 日志文件组的第一个 redo 日志文件的管理信息中，有两个 block 都存储了 checkpoint_lsn 的信息，取哪一个用来恢复页面呢？当然要取最近发生的那次 checkpoint 的信息。

我们可以将两个 block 的 checkpoint_no 读出来进行比对，谁的 checkpoint_no 更大，说明对应的 block 存储的就是最近一次的 checkpoint 信息。在拿到最近一次的 checkpoint 信息中的 checkpoint_lsn 值后，我们还可以拿到这个 checkpoint_lsn 值在 redo 日志文件组中的偏移量 checkpoint_offset。
...
redo 日志写入 block 是**顺序**写入的，即写满一个 block 后才会往下一个 block 中写入 redo 日志。

普通的 block 结构的 log block header 部分，有一个称为 **LOG_BLOCK_HDR_DATA_LEN** 的属性，它表示当前 block 里使用了多少字节的空间。对于填满 redo 日志的 block 来说，其 LOG_BLOCK_HDR_DATA_LEN 属性值永远为 512。对于未被填满 redo 日志的 block 来说，其 LOG_BLOCK_HDR_DATA_LEN 属性值不为 512。

因此，按顺序扫描 block，扫到的第一个 LOG_BLOCK_HDR_DATA_LEN 属性值不为 512 的 block，就是此次崩溃恢复中需要扫描的最后一个 block。
...
确定了恢复的起点与终点后，我们便可以**按顺序**从 redo 日志文件中取得 redo 日志开始恢复状态。InnoDB 对于这个**按顺序**的恢复过程，提出了一些方案来加快恢复进度。

* 方案一：使用哈希表。

  使用哈希链表（不知道哈希链表的请去学习数据结构），利用 redo 日志的 space ID 和 page number 属性计算出散列值，将其放入哈希链表的对应的槽中。若存在多个相同散列值的 redo 日志，就**按照 redo 日志生成的先后顺序**，将他们使用链表连接起来，然后放入哈希链表的对应的槽中。

  之后在恢复的时候，就可以利用哈希链表来加快恢复速度了。因为对同一个页面进行修改的 redo 日志，都被放到了同一个槽里，所以可以一次性的将一个页面修复好（省去了许多读取页面的随机 IO）。

  需要注意的是，在一次性修复一个页面的过程中，我们需要严格按照对应槽对应链表的顺序，进行修复，即按照该页面被修改而生成的redo 日志的先后顺序进行页面的修复。
...
* 方案二：跳过已经刷新到磁盘的页面。
  ...
  checkpoint_lsn 是一个分界点，当一个页面的 FIL_PAGE_LSN 属性大于这个分界点的时候，意味着这个页面在做了 checkpoint 之后发生了修改，那么这个页面曾经因为修改而产生的，已经被写入磁盘的 redo 日志，就没有必要再应用到这个页面的恢复工作中了，即 lsn 值小于该页面 FIL_PAGE_LSN 属性值的 redo 日志，没有必要用于该页面恢复过程。这样，我们就进一步
