...
* 服务器在**内存中**维护着一个全局变量（假设为全局变量 A），每当向某个包含隐藏列 row_id 的表中插入一条记录时，就会把该全局变量的值当作新记录的 row_id 列的值，并且把该全局变量自增 1。

* 当全局变量 A 的值是 256 的倍数时，就会将全局变量 A 的值刷新到系统表空间页号为 7 的页面的一个称之为 Max Row ID 的属性处。
