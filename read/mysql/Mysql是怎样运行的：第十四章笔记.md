...
* **LooseScan Execution Strategy （松散索引扫描）**。即查询语句子查询的查询列表，对应的列若可以使用到对应的索引。这样将查询语句转换为半连接查询后，若以子查询对应的表作为驱动表，扫描对应的索引，取出符合子查询查询条件的记录，去被驱动表中做匹配操作。这时，符合子查询查询条件的记录，可能会有重复，但是重复的值一定都聚集在一块，并且符合子查询条件的记录会按顺序排列。我们若不将驱动表符合子查询条件的每一条记录都去被驱动表中查询，而是只取值相同的记录的第一条去做匹配操作，这种方式，就被称之为**松散索引扫描**。
